...
class DeepLabResNetModel(BaseModel):
    def __init__(self, inbound, is_training, num_classes, atrous_blocks=22):
        super(DeepLabResNetModel, self).__init__(is_training)
        ...
        with tf.variable_scope('1a'):
            conv = layers.conv2d(self.inbound, 32, 7, 2, activation_fn=None)
            bn = layers.batch_norm(conv, activation_fn=tf.nn.relu, is_training=self.training)
            pool = layers.max_pool2d(bn, 3, 2)
        block_end = pool

        r2a_b1 = self._a_branch(block_end, 32, 1, 1, scope='2ab1')
        r2a_block = self._conv_block(block_end, (16, 16, 32), (1, 3, 1), (1, 1, 1), scope='2a')
        r2a_relu1 = self._add_block(r2a_b1, r2a_block, scope='2a')
        r2b_block = self._conv_block(r2a_relu1, (16, 16, 32), (1, 3, 1), (1, 1, 1), scope='2b')
        r2b_relu1 = self._add_block(r2a_relu1, r2b_block, scope='2b')
        r2c_block = self._conv_block(r2b_relu1, (16, 16, 32), (1, 3, 1), (1, 1, 1), scope='2c')
        r2c_relu1 = self._add_block(r2b_relu1, r2c_block, scope='2c')
        block_end = r2c_relu1

        r4a_b1 = self._a_branch(block_end, 64, 1, 1, scope='4ab1')
        r4a_block = self._aconv_block(r4a_b1, (16, 16, 64), (1, 3, 1), (1, 2, 1), scope='4a')
        r4a_relu = self._add_block(r4a_b1, r4a_block, scope='4a')
        r4b_relu = r4a_relu
        for i in range(1, atrous_blocks + 1):
            scope = '4b' + str(i)
            r4b_block = self._aconv_block(r4b_relu, (16, 16, 64), (1, 3, 1), (1, 2, 1), scope=scope)
            r4b_relu = self._add_block(r4b_relu, r4b_block, scope=scope)
        block_end = r4b_relu

        r5a_b1 = self._a_branch(block_end, 128, 1, 1, scope='5ab1')
        r5a_block = self._aconv_block(r5a_b1, (32, 32, 128), (1, 3, 1), (1, 4, 1), scope='5a')
        r5a_relu = self._add_block(r5a_b1, r5a_block, scope='5a')
        r5b_block = self._aconv_block(r5a_relu, (32, 32, 128), (1, 3, 1), (1, 4, 1), scope='5b')
        r5b_relu = self._add_block(r5a_relu, r5b_block, scope='5b')
        r5c_block = self._aconv_block(r5b_relu, (32, 32, 128), (1, 3, 1), (1, 4, 1), scope='5c')
